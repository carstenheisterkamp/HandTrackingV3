...
**Basierend auf:** OPTIMAL_WORKFLOW_V2_FINAL.md  
**Status:** 2026-01-09 - PHASE 0 IN PROGRESS  
**Aktueller Stand:** RGB-only Pipeline auf OAK-D (45 FPS Ziel)  
**Ziel:** 45 FPS @ 720p mit 2 VIPs + Person Detection auf Jetson  
...
### ✅ Was funktioniert
- [x] OAK-D Pro PoE Verbindung (stabil)
- [x] RGB Capture @ 1080p/30 FPS
- [x] Zero-Copy Pipeline (Pinned Memory)
- [x] MJPEG Preview Server (mit hasClients() Check)
- [x] Stereo Depth Infrastructure (CUDA Kernel)
- [x] OSC Output Framework

### 🔄 Phase 0 Status (2026-01-09)
**Problem erkannt:** Zwei NNs (Palm + Landmarks) auf OAK-D → 19 FPS Bottleneck!

**Korrektur laut OPTIMAL_WORKFLOW_V2_FINAL.md:**
```
OAK-D Pro PoE (Device):
  ✅ RGB Capture @ 720p/45 FPS
  ✅ Stereo Depth @ 400p/20 FPS  
  ✅ ObjectTracker @ 45 FPS
  ❌ KEINE NNs! (Memory-Constraint: 2.5 MB CMX Limit)

Jetson Orin Nano (Host):
  ✅ Person Detection (YOLOv8n @ 12 FPS) - TensorRT
  ✅ Hand Landmarks (MediaPipe @ 30 FPS) - TensorRT
  ✅ Gesture Classifier @ 15 FPS
```

**Aktueller Fix:** RGB-only Pipeline (keine NNs auf OAK-D)
- Erwartet: 45 FPS Device Output
- NNs werden in Phase 1 auf Jetson implementiert (TensorRT)

### ❌ Was fehlt für OPTIMAL_WORKFLOW
- [ ] 45 FPS Device Output (Phase 0 - IN PROGRESS)
- [ ] Person Detection auf Jetson (Phase 1)
- [ ] Hand Landmarks auf Jetson (Phase 1)  
- [ ] ObjectTracker auf OAK-D (Phase 2)
- [ ] Multi-VIP Support (Phase 2)
- [ ] ROI-System (Phase 2)

---

## 🚀 PHASE 0: RGB-Only Pipeline (19 → 45 FPS)

**Ziel:** Stabile 45 FPS RGB Baseline auf OAK-D  
**Status:** 🔄 IN PROGRESS (2026-01-09)  
**Erwartet:** 45 FPS ohne NNs auf Device  

### Das Problem

**Vorher:** 2 NNs auf OAK-D (Palm + Landmarks) → **19 FPS**
- Palm Detection: ~50ms Inference
- Landmarks: ~50ms Inference  
- Beide teilen sich VPU (Myriad X) → Bottleneck!

**Laut OPTIMAL_WORKFLOW_V2_FINAL.md:**
```
OAK-D soll NUR:
- RGB Capture @ 720p/45 FPS
- Stereo Depth @ 400p/20 FPS
...
Jetson soll ALLE NNs ausführen:
- Person Detection (YOLOv8n @ 12 FPS) - TensorRT
- Hand Landmarks (MediaPipe @ 30 FPS) - TensorRT
- Gesture Classifier @ 15 FPS
```
...
**RGB-only Pipeline auf OAK-D:**
```cpp
// PipelineManager.cpp - Keine NNs mehr!
auto rgbQueue = rgbOutput->createOutputQueue(4, false);
queues_["rgb"] = rgbQueue;

// InputLoop.cpp - Nur RGB empfangen
imgFrame = queue->tryGet<dai::ImgFrame>();
frame->nnData.clear();   // Wird von Jetson gefüllt (Phase 1)
frame->palmData.clear(); // Wird von Jetson gefüllt (Phase 1)
```

### Acceptance Criteria

- [ ] **Device FPS: 45** (RGB-only, keine NNs)
- [ ] **Latenz: < 30ms** (Camera → Host)
- [ ] **Stabilität:** Keine Frame-Drops über 5 Minuten
- [ ] MJPEG Preview funktioniert noch
...
./HandTrackingV3
# Erwartete Logs:
# [INFO] 📹 DEVICE FPS: 45.x (OAK-D RGB Stream)
# [INFO]   ✅ Device FPS EXCELLENT! (Target: 45 FPS)
```

### Nächste Schritte nach Phase 0

1. **Phase 1:** TensorRT Inference auf Jetson
   - YOLOv8n Person Detection
   - MediaPipe Hand Landmarks
   
2. **Phase 2:** ObjectTracker auf OAK-D
   - Tracking überbrückt Detection-Lücken
   - Multi-VIP Support
# Erwartung: 28-30 FPS
